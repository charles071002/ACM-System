    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-black/60 backdrop-blur-sm">
      <div className="bg-white rounded-[2.5rem] w-full max-w-sm overflow-hidden shadow-2xl animate-scale-up border-4 border-yellow-500 flex flex-col max-h-[85vh]">
        ...
        <div className="bg-[#1a3683] p-8 flex flex-col items-center text-white text-center relative flex-shrink-0">
          ...
          <h3 className="text-2xl font-black tracking-tight text-[#eab308] uppercase leading-none">
            Submission History
          </h3>
          <p className="text-[10px] font-bold opacity-90 uppercase tracking-[0.1em] mt-2">
            {professor.name}
          </p>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-white">
          {history.length > 0 ? (
            history.map((record, index) => (
              <div
                key={record.id}
                className="bg-[#f0f7ff] border-2 border-[#dbeafe] rounded-2xl p-4 flex items-center justify-between shadow-sm animate-fade-in"
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex flex-col">
                    ...
                    <p className="text-[11px] font-bold text-blue-800/80 flex items-center gap-1.5">
                      ...
                      {new Date(record.clearedAt).toLocaleDateString()} at{' '}
                ...
                <button
                  ...
                  className="w-10 h-10 flex items-center justify-center bg-red-50 text-red-500 rounded-full transition-all active:scale-90 hover:bg-red-100"
                >
        ...
        <div className="p-6 bg-white border-t border-gray-100 flex-shrink-0">
          <button
            ...
            className={`w-full py-3.5 border-2 rounded-2xl font-black text-xs uppercase tracking-widest shadow-sm transition-all flex items-center justify-center gap-3 active:scale-95 ${
              history.length === 0
